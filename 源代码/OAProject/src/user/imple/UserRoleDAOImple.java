...
public class UserRoleDAOImple extends HibernateDaoSupport implements UserRoleDAO{
	...
	public TUserRole findByUserid(int userid){
		String hql=" from TUserRole t where t.id.userid=? ";
	...
	public boolean isUser(int rid){
		String hql="from TUserRole t where t.id.roleid=?";
	...
	public int deleteEmail(String username) {
		String hql="from TPostUser where id.suserid=?";
		log.debug("getting TRoleMenu instance with id: " + username);
		try {
			this.getHibernateTemplate().deleteAll(findEmail(username));
			return 1;
		} catch (RuntimeException re) {
			log.error("get failed", re);
			throw re;
	...
	public int deletePost(int userid) {
		log.debug("getting TRoleMenu instance with id: " + userid);
		try {
			this.getHibernateTemplate().deleteAll(findPost(userid));
			return 1;
		} catch (RuntimeException re) {
			log.error("get failed", re);
			throw re;
	...
	public List findPost(int userid){
		String hql="from TPostUser where id.suserid=?";
		log.debug("getting TRoleMenu instance with id: " + userid);
		try {
	...
	public List findEmail(String username){
		String hql="from TReceiveemail where id.username=?";
		log.debug("getting TRoleMenu instance with id: " + username);
		try {
